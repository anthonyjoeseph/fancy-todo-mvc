...
Formally, `redux-observable` is a referentially transparent functional architecture - an [IO entry point](https://dev.to/anthonyjoeseph/taskeither-vs-fluture-4e0n). Its history is edifying because it took the long way to get there (as opposed to something like Elm). Understanding that journey can help understand some of the stranger points about it and some of the documentation and opinion pieces surrounding it.
